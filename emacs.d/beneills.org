...
Open downloads directory, if not already open:
#+BEGIN_SRC emacs-lisp
  (let ((downloads-path "~/tmp/downloads"))
    (unless (get-file-buffer downloads-path)
      (find-file downloads-path)))
#+END_SRC

Open current project directories, if not already open:
#+BEGIN_SRC emacs-lisp
  (setq current-projects '("~/files/inbox/highlighting"))

  (dolist (path current-projects)
